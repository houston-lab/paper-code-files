...
angles = arrayfun(@getAngles,tracksFinal,'UniformOutput',false);
angles = horzcat(angles{:});

% get rid of NaNs and linearize the vector, save raw angle data (radians)
% to projData, run circStats // DWH 2016
projData.frame2frameDispPix=frame2frameDispPix(~isnan(frame2frameDispPix(:)));
projData.angles=angles(~isnan(angles(:)));
projData.circStats=circ_statsmod(projData.angles');
