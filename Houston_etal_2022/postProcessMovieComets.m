...
% Set up the output file; added _angle_histograms //DWH
...
for i = p.ChannelIndex
    ...
    outFilePaths{4,i} = [p.OutputDirectory filesep 'channel_' num2str(i) '_angStats.txt'];
    outFilePaths{5,i} = [p.OutputDirectory filesep 'channel_' num2str(i) '_histograms'];
    outFilePaths{6,i} = [p.OutputDirectory filesep 'channel_' num2str(i) '_angle_histograms'];
end
...
for i = p.ChannelIndex
    ...
    % new code for printing circular statistics into a text file //DWH
    % 2014 and 2016
    angFile = outFilePaths{4, i};
    angData = struct2cell(projData.circStats);
    angName = fieldnames(projData.circStats);
    fid=fopen(angFile,'w+');
    for j=1:numel(angName)
        fprintf(fid,'%s\t%g\n',angName{j}, angData{j});
    end
    utrackPlotAngles(projData.angles, outFilePaths{6, i});
    fclose(fid);

    
    if p.makeHist==1
        plusTipMakeHistograms(M, outFilePaths{5, i});
